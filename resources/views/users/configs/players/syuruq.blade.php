...
@section('title', 'Config Player Syuruq')
...
<div class="container">
    ...
    <div class="row my-4">
        <div class="col-md-4">
            <h3>{{ __("Fitur Syuruq") }}</h3>
            <p>{{ __("Pengaturan fitur syuruq.") }}</p>
        </div>
        <div class="col-md-8">
            <div class="card">
                <div class="card-body p-4">
                    <form action="{{ route('config.player.syuruq.update', ['show']) }}" method="POST">
                        @csrf

                        <div class="form-check form-switch">
                            <input class="form-check-input" id="with_syuruq" name="with_syuruq" type="checkbox" {{ $configPlayer->with_syuruq ? 'checked' : '' }} value="1"/>
                            <label class="form-check-label" for="with_syuruq">{{ __('Aktifkan fitur ini') }}</label>
    ...
    <div class="{{ !$configPlayer->with_syuruq ? 'd-none' : '' }}">
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-body p-4">
                        ...
                        <form method="POST" action="{{ Route::has('config.player.syuruq.update') ? route('config.player.syuruq.update', ['key' => 'prayer']) : '#' }}#{{ $configPrayer->name }}">
